...
class CyberBlitzUI:
    def __init__(self, master):
        self.master = master
        master.title("Cyber Blitz - Advanced Web Vulnerability Scanner")
    ...
    def create_menu(self):
        ...
        menubar = Menu(self.master, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
        
        # File menu
        file_menu = Menu(menubar, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
        ...
        # Scan menu
        scan_menu = Menu(menubar, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
        ...
        # Tools menu
        tools_menu = Menu(menubar, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
        ...
        # Help menu
        help_menu = Menu(menubar, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
    ...
    def create_header(self):
        ...
        # Logo placeholder
        self.logo_label = ttk.Label(header_frame, text="⚡", font=('Arial', 24), style='TLabel')
        ...
        self.title_label = ttk.Label(title_frame, 
                                   text="Cyber Blitz - Advanced Web Vulnerability Scanner", 
                                   font=('Segoe UI', 16, 'bold'),
                                   style='TLabel')
        self.title_label.pack(anchor='w')
        
        self.subtitle_label = ttk.Label(title_frame, 
                                      text="Comprehensive Security Assessment Tool v3.0", 
                                      font=('Segoe UI', 9),
                                      style='TLabel')
        ...
        self.export_btn = ttk.Button(action_frame, text="Export", command=self.show_export_menu)
        self.export_btn.pack(side=tk.LEFT, padx=5)
        
        self.settings_btn = ttk.Button(action_frame, text="Settings", command=self.show_settings)
        self.settings_btn.pack(side=tk.LEFT, padx=5)
        
        self.docs_btn = ttk.Button(action_frame, text="Docs", command=self.open_documentation)
    ...
    def create_results_panel(self):
        ...
        self.tree = ttk.Treeview(vuln_frame, columns=('time', 'type', 'url', 'severity', 'confidence'), show='headings')
    ...
    def create_dashboard(self):
        ...
        self.dashboard_fig = Figure(figsize=(8, 4), dpi=100, facecolor='#1e1e2e')
        self.severity_fig = Figure(figsize=(4, 4), dpi=100, facecolor='#1e1e2e')
        ...
        # Layout charts
        self.dashboard_canvas.get_tk_widget().pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.severity_canvas.get_tk_widget().pack(side=tk.RIGHT, fill=tk.BOTH)
    ...
    def update_dashboard(self, data=None):
        ...
        if data['timeline']:
            ...
            ax1.fill_between(times, counts, color='#4a6da7', alpha=0.1)
    ...
    def add_vulnerability(self, vuln):
        ...
        self.tree.tag_configure('critical', background='#2a2a3a', foreground='#ff3860')
        self.tree.tag_configure('high', background='#2a2a3a', foreground='#ff7c4d')
        self.tree.tag_configure('medium', background='#2a2a3a', foreground='#ffdd59')
        self.tree.tag_configure('low', background='#2a2a3a', foreground='#48c774')
        self.tree.tag_configure('info', background='#2a2a3a', foreground='#209cee')
    ...
    def show_export_menu(self):
        ...
        menu = tk.Menu(self.master, tearoff=0, bg='#2a2a3a', fg='#e0e0e0', activebackground='#3a3a4a')
        ...
        menu.add_command(label="SARIF", command=lambda: self.export_results('sarif'))
        ...
        if file_path:
            try:
                ...
                if format == 'json':
                    ...
                    self.export_sarif(file_path, findings)
...
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; margin: 20px; background-color: #1e1e2e; color: #e0e0e0; }
        h1 { color: #4a6da7; }
        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        th, td { padding: 8px; text-align: left; border-bottom: 1px solid #3a3a4a; }
        ...
        .low { color: #48c774; }
        .info { color: #209cee; }
        .summary { background-color: #2a2a3a; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
...
"""
    ...
    def export_pdf(self, file_path, findings):
        ...
        try:
            ...
            messagebox.showerror("Error", "PDF export requires fpdf module. Install with: pip install fpdf2")
            return
        ...
        # Title
        pdf.cell(200, 10, txt="Cyber Blitz Vulnerability Scan Report", ln=1, align='C')
        pdf.ln(10)
        ...
        pdf.cell(200, 6, txt=f"Scan Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=1)
        pdf.cell(200, 6, txt=f"Target URL: {self.url_entry.get()}", ln=1)
        pdf.cell(200, 6, txt=f"Total Findings: {len(findings)}", ln=1)
        pdf.ln(5)
        
        # Findings table header
        pdf.set_font("Arial", 'B', 10)
        pdf.cell(20, 6, txt="Time", border=1)
        pdf.cell(30, 6, txt="Type", border=1)
        pdf.cell(100, 6, txt="URL", border=1)
        pdf.cell(20, 6, txt="Severity", border=1)
        pdf.cell(20, 6, txt="Confidence", border=1, ln=1)
        
        # Findings rows
        pdf.set_font("Arial", size=8)
        for item in findings:
            pdf.cell(20, 6, txt=item['time'], border=1)
            pdf.cell(30, 6, txt=item['type'], border=1)
            pdf.cell(100, 6, txt=item['url'], border=1)
            pdf.cell(20, 6, txt=item['severity'], border=1)
            pdf.cell(20, 6, txt=item['confidence'], border=1, ln=1)
        
        pdf.output(file_path)

    def export_sarif(self, file_path, findings):
            ...
            "runs": [{
                "tool": {
                    "driver": {
                        "name": "Cyber Blitz",
                        "version": "3.0",
                        "informationUri": "https://github.com/cyberblitz/scanner",
                        "rules": []
                    }
                },
                "results": []
        ...
        for finding in findings:
            rule_id = f"CB-{finding['type'].replace(' ', '-')}"
            if rule_id not in rule_ids:
                sarif['runs'][0]['tool']['driver']['rules'].append({
                    ...
                    "defaultConfiguration": {
                        "level": self.map_sarif_severity(finding['severity'])
                    }
                })
            ...
            sarif['runs'][0]['results'].append({
                "ruleId": rule_id,
                "message": {
                    "text": f"{finding['type']} found at {finding['url']}"
                },
                "locations": [{
                    "physicalLocation": {
                        "artifactLocation": {
    ...
    def map_sarif_severity(self, severity):
    ...
    def show_settings(self):
        ...
        # Save button
        ttk.Button(main_frame, text="Save", command=settings.destroy).pack(pady=10)
    ...
    def show_about(self):
        ...
        about.configure(bg='#1e1e2e')
        
        ttk.Label(about, text="Cyber Blitz Vulnerability Scanner", font=('Segoe UI', 14, 'bold')).pack(pady=10)
        ttk.Label(about, text="Version 3.0").pack()
        ttk.Label(about, text="Advanced Security Assessment Tool").pack(pady=10)
        ttk.Label(about, text="© 2023 Cyber Blitz Project").pack()
        ttk.Label(about, text="License: MIT").pack(pady=10)
    ...
    def show_tech_stack(self):
        ...
        try:
            ...
            tech_window.configure(bg='#1e1e2e')
    ...
    def show_port_scanner(self):
        ...
        try:
            ...
            port_window.configure(bg='#1e1e2e')
    ...
    def show_repeater(self):
        ...
        if url:
            ...
            self.req_text.insert(tk.END, "User-Agent: CyberBlitz/3.0\n")
...
class ScannerEngine:
    ...
    def get_modules_for_scan_type(self, scan_type):
        ...
        scan_types = {
            'Full Scan': ['xss', 'sqli', 'lfi', 'rce', 'ssrf', 'xxe', 'idor', 'csrf', 'ssti', 'tech'],
            'Quick Scan': ['xss', 'sqli', 'lfi', 'tech'],
            'XSS Only': ['xss'],
            'SQLi Only': ['sqli'],
            'LFI/RFI': ['lfi'],
            'RCE': ['rce'],
